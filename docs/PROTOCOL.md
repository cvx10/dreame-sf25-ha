...
| 2/11 | Time remaining (min) | counts down 1/min; **frozen while paused**; 0 on stop; **360 for drying, 90 for cleaning** at fresh start. **Adaptive in BOTH directions** (2026-07-10): the device targets a dryness threshold (3/2 humidity ~24), not a fixed duration — observed a clean `1 → 119` extension (humidity still ~25, no other property change) AND, later the same cycle, drying ending early with 55 min still on the clock (humidity reached ~24). Wetter/bigger loads run longer. |
